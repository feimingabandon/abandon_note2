...
- GitCode 镜像及国内下载仓库：<https://gitcode.com/zou-feiming/abandon_note2>
- 主分支：`main`
- 当前版本：`0.9.1`
...
| GitCode 持续跟踪 GitHub `main` | 已配置并完成首次导入 | 后续在 GitCode 仓库设置中查看同步状态 |
| 手动运行三平台候选包构建 | 已真实跑通 | Windows x64、macOS x64、macOS arm64 均成功 |
| 本地 Windows NSIS 实际打包 | 已真实跑通 | C++ DLL 已进入安装包 |
| 应用内版本检测和 Windows 全量下载 | 已实现并通过自动测试 | GitHub 已公开 `v0.9.0`；需用旧版测试下一次公开更新 |
| 推送版本标签后自动生成 GitHub Draft Release | `v0.9.1` 已生成 Draft | Draft 尚未公开 |
| 正式发布 GitHub Release 后同步附件到 GitCode Release | 配置已完成，尚未真实发布验证 | 只有点击 GitHub 的“发布版本（Publish release）”后才会触发 |

所以，GitHub构建发布和GitCode代码镜像已经跑通；GitCode Release附件同步仍需首次真实验证。

## 2. 最重要的概念：提交不等于推送，推送不等于发布

- `git commit`：只在本机生成一个提交，不会触发 GitHub 的任何功能。
- `git push origin main`：提交到达 GitHub `main` 后触发普通 CI；GitCode 通过自身的持续跟踪拉取更新。
- 推送普通代码不会生成安装包，也不会发布版本。
- 手动运行候选构建只生成供测试的临时文件，不会创建 Release。
- 推送与 `package.json` 版本一致的 `v*` 标签，才会生成 Draft Release。
- Draft Release 不公开，也不会同步到 GitCode Release。
- 最后点击“发布版本（Publish release）”，才算 GitHub 正式发布，并触发 GitCode Release 同步。

```mermaid
flowchart TD
  A["本机 git commit"] --> B["git push origin main"]
  B --> C["CI：Windows/macOS 检查"]
  B --> D["GitCode 持续跟踪：拉取 main"]
  B -. "不会自动打包" .-> E["等待下一次提交"]
  ...
  M --> O["同步说明、附件到 GitCode Release"]
  O --> P["用户可从 GitCode/GitHub 更新"]
```
...
只有进入 GitHub `main` 的提交才会成为 GitCode 持续跟踪的正式同步对象。Pull Request
中的未合并提交不会出现在 GitCode `main`。
...
3. 左侧会看到：
   ...
   - `Sync Release to GitCode`（只负责已公开 Release，不负责 `main`）
4. 点击 `CI`，打开本次运行记录。
5. 页面中应看到两个任务：
...
- 合并到 `main`：再次运行 `CI`，GitCode 持续跟踪随后拉取该提交。

## 4. GitHub 的代码怎么同步到 GitCode

代码同步由 GitCode 仓库设置中的持续跟踪（Pull 镜像）完成，方向固定为：

```text
GitHub → GitCode
```

GitHub Actions 不再使用令牌重复推送 `main` 或标签，避免持续跟踪和 Actions 同时写入
GitCode。`GITCODE_TOKEN` 只用于创建 Release、更新说明和上传附件。

`GITCODE_TOKEN` 只保存在 GitHub：

1. GitHub 仓库“设置（Settings）”
2. “机密和变量（Secrets and variables）”
3. “操作（Actions）”
4. “仓库机密（Repository secrets）”
5. Secret 名称：`GITCODE_TOKEN`

令牌不会写进应用，也不会发给最终用户。

查看同步结果时，直接打开 GitCode 的 `main` 分支并与 GitHub 最新提交号比较，也可以在
GitCode 仓库镜像设置中查看最近同步状态。不要直接在 GitCode `main` 上开发或提交，以免
持续跟踪出现冲突。
...
6. 在 GitHub → “操作（Actions）”中等待 `CI` 和 `Sync Release to GitCode` 成功。

### 6.2 可选：先构建一次候选包

本步骤直接在 GitHub 网页完成，不需要本地终端。按照第 5 节点击：

`Actions` → `Build release candidates` → `Run workflow` → `main` → `Run workflow`

下载三个构件交给测试人员。确认主要功能、安装位置选择、覆盖安装、数据保留以及 macOS 打开方式。

本步骤是可选项，不是创建正式标签的强制前提；但首次发布或更新安装、原生模块相关代码时强烈建议执行。

### 6.3 通过 IntelliJ IDEA 创建并推送版本标签

确认测试通过后：

1. 打开 `package.json`，确认准备发布的版本号；
2. 打开 IDEA 底部 `Git` →“日志（Log）”；
3. 确认最新提交同时标有 `HEAD`、`main`、`origin/main`，并确认 GitHub `CI` 已成功；
4. 右键准备发布的最新提交；
5. 点击“新建标记…（New Tag…）”；
6. 标签名称填写 `v` 加 `package.json` 版本，例如 `v0.9.0`；
7. 如果有“消息（Message）”字段，填写 `Abandon Note v0.9.0`；
8. 点击顶部 `Git` →“推送…（Push…）”；
9. 启用“推送标签（Push Tags）”，确认远程仓库为 `origin`；
10. 点击“推送（Push）”。

IDEA 中文版中的“标记”就是 Git `Tag`。不同 IDEA 版本的“推送标签”可能位于推送窗口、更多选项菜单，或“推送”按钮旁的下拉菜单中。
...
- `Sync Release to GitCode` 收到 `release: published` 事件；
- GitCode Release 同步任务开始。

“发布版本（Publish release）”是对外发布动作。点击前必须确认附件和说明。

## 7. 正式 Release 怎么同步到 GitCode

GitHub Release 发布后，`Sync published Release to GitCode` 执行：

1. “验证 GitCode 访问（Verify GitCode access）”
   - 使用 `GITCODE_TOKEN` 验证目标仓库。
2. 持续跟踪应已经将同名标签同步到 GitCode，工作流不会重复推送标签。
...
5. “同步 GitCode Release（Synchronize GitCode Release）”
   - 按标签查询 GitCode 是否已有 Release。
   - 没有则创建；已有则更新标题和说明。
   - 为每个附件取得GitCode临时上传地址并使用 `PUT` 上传。
   - 新附件上传成功后清理同名旧附件，上传失败时保留已有下载入口。
...
4. `Sync published Release to GitCode` 必须为“成功（Success）”
5. 打开 GitCode 仓库 → “发行版（Releases）”
6. 确认版本、说明和附件
7. 实际下载一个文件，确认链接可用
...
2. 打开失败的 `Sync Release to GitCode` 运行记录；
...
- 点击 `Publish release`；
- GitHub Release 公开；
- `Sync published Release to GitCode` 成功；
- GitCode Release 的版本、说明、附件检查通过；
- Windows 和两种 Mac 至少各完成一次下载/打开测试。

只有做到这里，才算 GitHub 和 GitCode 双平台发布完成。

## 9. 现有 `v0.9.1` Draft 的特殊情况

`v0.9.1` 标签和 Draft附件已经在切换GitCode前构建，因此：

- 发布 `v0.9.1` 后，新的默认分支工作流仍可把附件同步到GitCode；
- 已构建的 `v0.9.1` 客户端仍运行旧的更新源逻辑；
- GitCode/GitHub并行查询逻辑从下一次重新构建的版本开始生效；
- 不要移动、删除或重建现有标签，除非另行明确授权。
...
1. GitCode：`https://api.gitcode.com/api/v5/repos/zou-feiming/abandon_note2/releases/latest`
2. GitHub：`https://api.github.com/repos/feimingabandon/abandon_note2/releases/latest`

选择规则：

- 选择两边版本号较高的稳定版；
- 同版本优先使用附件完整的 GitCode Release；
- GitCode 同版本附件不完整时使用 GitHub 附件；
- 任一来源失败时仍可使用另一个来源；
- GitCode 尚无 Release 时返回的 HTTP 400 按“未发布”处理。

客户端只访问公开 API，不包含 `GITCODE_TOKEN` 或 GitHub Token。
...
| `error` | GitCode 和 GitHub 都无法返回有效稳定版本 | 显示错误，但保留手动地址 |
...
1. 使用版本较高且当前系统所需附件完整的来源；同版本正常优先 GitCode。
...
- renderer 只通过受限 preload API 请求“检查、下载、安装、打开 GitCode/GitHub”；
- 主进程只接受主窗口发来的更新 IPC；
...
- GitCode 是同版本下的国内优先下载源；
- GitHub 是构建和发布源；
- 两边同时查询并选择较高版本，任一失败时使用另一个；
- Windows 只做用户点击触发的全量半自动更新；
- macOS 只做手动更新；
- 无论成功失败都要显示 GitCode/GitHub 双链接；
- 必须明确告诉用户当前系统应下载哪个文件；
- 不做差分下载；
- 不自建更新服务器。
...
- [ ] `Sync published Release to GitCode` 成功
- [ ] GitCode Release 的标签、说明和附件齐全
- [ ] GitCode/GitHub 至少各测试一个下载链接
...
- GitHub → GitCode 同步：`.github/workflows/sync-gitcode.yml`
- GitCode Release 附件脚本：`.github/scripts/sync-gitcode-release.sh`
