...
- 📄 **便签报表** — 按日期和状态筛选便签，预览后导出为 TXT 或 Excel 文件
...
欢迎加入“便签测试群”，参与新版本测试、反馈问题和交流使用体验。二维码如已失效，可通过 Issue 或邮件联系作者获取新的入群方式。

<div align="center">
  <img src="docs/assets/wechat-test-group-qr.png" width="320" alt="便签测试群微信二维码" />
</div>
...
本次更新重点调整便签的持续方式，丰富正文与图片的展示，并改进窗口交互和本地诊断。

#### 便签与日历

- 新增便签备注，可在快速编辑中分别修改正文和备注，并单独调整备注字号。
- 支持为正文中的选中文字设置颜色；颜色会显示在便签卡片、搜索结果及月、周视图中。
- 新增“标签颜色”开关，控制有标签便签的正文和日历横条是否使用标签颜色。
- 支持创建只有图片、没有正文的便签；卡片可轮播图片，月、周视图也可打开图片并前后切换。
- 持续方式改为“仅当天、指定天数、持续到完成”。持续到完成的便签会在日历中延续，完成后固定结束日期；报表会显示相应持续信息。
- 移除旧的“历史未完成便签移至今天”和“自动移至今天”功能，改用新的持续方式。旧便签不会因升级自动变成“持续到完成”。
- 日历的当日便签预览可直接执行提前开始、标记完成或重新进行；月、周工具栏也调整了天气和循环便签预览入口。
- 循环模板可设置开始时间和可选结束时间，仅在有效期内生成便签。

#### 窗口与设置

- 重做主视图与灵动岛的切换和原生毛玻璃动画；重新启动或从托盘打开时进入完整主视图，不再跨启动保存灵动岛状态。
- 改进窗口拖动、贴边隐藏、层级切换及恢复过程中的状态同步。
- 新增“截图时隐藏主视图”开关，可选择是否将应用窗口截入图片，并改进截图窗口异常退出后的恢复。
- “恢复默认设置”现在也会恢复图标、天气、远程与隐私等全局共享设置；确认窗口会说明影响范围。

#### 诊断与帮助

- 日志支持导出最近一小时或全部记录；新增操作关联和保存结果记录，便于定位便签、设置及窗口问题。
- 修复帮助中心快捷入口，使其指向当前仍存在的功能说明。

### Abandon Note 1.2.0

当前版本修改颇多，本想着再打磨一阵。但由于其中若干改进建议被反复提及，因此决定提前发布。若使用过程中遇到问题，可发送邮件至 1160653906@qq.com，或加入便签测试群反馈。Windows 10 用户使用灵动岛时可能遇到小问题，双击缩放过程中偶尔会出现闪烁，目前仍在排查。最后，感谢使用！

本次版本新增多项便签管理、日历交互与窗口能力，并重点加强数据保护和 Windows 原生交互稳定性。

#### 主要更新

- 新增灵动岛模式，支持双击收起或展开、长按拖动、位置记忆及便签状态操作。
- 新增始终置底模式，可在“始终置顶、普通窗口、始终置底”之间切换。
- 三种主视图统一新增视图切换入口，并支持全局快捷键显示或隐藏当前窗口。
- 新增历史便签补录，可直接为过去日期创建便签。
- 新增历史未完成便签移至今天，支持昨天、最近三天、全部历史、自定义范围及逐条选择。
- 新增“自动移至今天”，可每天自动移动昨天未完成的普通单日便签。
- 列表、月视图和周视图支持双击快速编辑正文，桌面便利贴也可直接编辑。
- 新增未保存草稿保护和编辑冲突处理，切换视图或退出时不再轻易丢失正在编辑的内容。
- 已删除便签现在可以通过搜索结果恢复，并保留正文、标签和图片。
- 循环模板新增自然季度规则、未来生成预览，以及错过当天生成时间后的启动补生成。
- 月视图升级为完整的六行日历，并新增日期右键菜单、当日便签预览和日期格快速创建。
- 月、周日期列表改为独立开关，支持拖动调整宽度，并分别记忆两个视图的宽度。
- 新增便签报表导出，支持最长 366 天范围以及 TXT、Excel 格式。
- 优化天气定位和城市选择，便签列表、月视图及周视图均可显示对应日期的天气。
- 新增导航栏图标大小和黑白颜色设置，并支持主窗口内侧边框及更大的日历字体。
- 软件通知和更新说明支持 Markdown、表格、代码、链接及 HTTPS 图片。
- 帮助中心新增分类导航和内容搜索，并补充本版本新增功能的使用说明。

#### 体验与稳定性优化

- 优化灵动岛收起与展开、毛玻璃恢复、窗口显示和多显示器缩放切换。
- 加强贴边隐藏、触边确认、全屏检测和窗口层级恢复。
- 优化月、周视图切换、刷新、便签增删、排序及日期侧栏动画。
- 加强桌面便利贴的正文同步、状态恢复和异常退出恢复。
- 优化中文输入法、下拉面板、标签选择器、键盘操作和焦点恢复。
- 优化大量便签、附件和图片缩略图的加载性能。
- 完善异常恢复机制，窗口崩溃或核心任务失败时提供重载、重试和诊断入口。
- 加强诊断日志，记录窗口位置、显示器、DPI、缩放比例、毛玻璃和贴边状态等信息。
