...
.sr-control--actionable { cursor: pointer; }
.sr-control:disabled { pointer-events: none; }
.sr-visual {
  display: block;
  width: 20rem;
  height: 20rem;
...
.sr-control--waiting .sr-track { opacity: 0.42; }
...
@keyframes sr-play-launch {
  0% { transform: translateX(-0.8px) scale(0.92); opacity: 1; color: #0a84ff; }
  35% { transform: translateX(-1.1px) scale(0.96); opacity: 1; }
  72% { transform: translateX(4.2px) scale(1); opacity: 1; }
  100% { transform: translateX(6px) scale(0.72); opacity: 0; }
}
