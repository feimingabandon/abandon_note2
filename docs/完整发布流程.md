...
1. 打开失败日志，确认失败发生在哪一步；
   - `Download GitHub Release assets` 很快完成、但 `Uploading ... to Gitee` 长时间低速：
     问题位于 GitHub Runner 到 Gitee 的大文件上传路径；
   - 小附件成功、200MB 以上安装包失败：优先检查 Gitee 大附件限制和跨境网络质量；
