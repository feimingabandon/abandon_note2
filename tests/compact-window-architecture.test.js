...
describe('compact single-window architecture', () => {
  ...
  it('restores BrowserWindow geometry before releasing a failed renderer scene transaction', () => {
    ...
    expect(blurEngine).toContain('IsWindowVisible(self->m_parentHwnd.load()))')
    expect(blurEngine).toContain('const HRESULT presented = DwmFlush()')
    expect(blurEngine).toContain('RecordNativeFailure("apply-config-dwm-flush", presented)')
