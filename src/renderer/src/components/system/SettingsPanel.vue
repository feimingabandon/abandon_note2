...
onMounted(async () => {
  ...
  // 面板尺寸决定进场起点，必须先读取保存值再启动动画；否则月/周视图会先按
  // 默认 40% 宽度出现，随后在动画途中瞬间跳到用户保存的宽度。
  const secondaryLoads = Promise.all([loadRemoteHealth(), loadHolidayDataStatus()])
