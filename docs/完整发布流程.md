...
1. 仓库 → “操作（Actions）”
2. 左侧 `Build release candidates`
3. 点击“运行工作流（Run workflow）”
...
CI 成功并且候选包测试完成后，确保本机 `main` 是最新状态：
...
普通 `main` push 不会打包，推送 `v0.9.1` 才会开始正式三平台打包。
...
2. `Sync to Gitee`；
3. 打开失败运行；
4. 点击“重新运行任务（Re-run jobs）”；
5. 选择“重新运行失败的任务（Re-run failed jobs）”；
6. 再检查 Gitee Release。
...
确认要开始第一次 Draft 后，只需创建并推送标签：

```powershell
git switch main
git pull --ff-only origin main
git tag -a v0.9.0 -m "Abandon Note v0.9.0"
