...
int Blur_SyncGeometryAndWait(int timeoutMs) {
    const DWORD boundedTimeout = static_cast<DWORD>(std::clamp(timeoutMs, 1, 5'000));
    return BlurEngine::Engine::Instance().SyncGeometryAndWait(boundedTimeout) ? 1 : 0;
}
