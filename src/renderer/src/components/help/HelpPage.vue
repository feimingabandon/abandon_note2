            ...
            <div :ref="(el) => registerAnchor('notes-create', el)" data-anchor-id="notes-create">
              <HelpFigureBlock title="新建便签">
                ...
                <ol class="help-anno-list">
                  ...
                  <li>
                    <strong>生效时间</strong> ——
                    默认「立即生效」，创建后直接进入「进行中」。也可选择当前或过去时间补录历史便签，或选择至少
                    2
                    分钟后的未来时间进行预约；预约到点前保持「初始化」，到点后由后台调度器自动切为「进行中」。
                  </li>
                  <li>
                  ...
                  <li>
                    <strong>系统提醒</strong> —— 生效时间到达时弹出操作系统通知。仅在选择至少 2
                    分钟后的未来生效时间时可开启；「立即生效」和历史补录均不发送提醒。<br />
          ...
          >
            ...
            <div class="help-figures">
              <HelpFigureBlock title="创建与频率">
                <template #figure>
                  <div class="help-fig-seg help-fig-seg--wrap">
                    ...
                    ><span>每季度</span><span>每年</span>
                  </div>
                </template>
                <p>
                  填写正文，选择每天、每周、每月、自然季度或每年的生成规则，并预设生成时刻、系统提醒、便签置顶和标签。自然季度可以选择季度内第几个月及具体日期；下次生成时间会实时预览，保存前先确认是否符合预期。
                </p>
              </HelpFigureBlock>
              <HelpFigureBlock title="暂停、恢复与删除">
                ...
                <p>
                  右键模板可以暂停、恢复或删除。暂停期间不再生成，恢复后从下一周期继续；普通删除可恢复，彻底删除永久移除。应用未运行时错过的历史周期不会在下次启动时批量补齐，但当天已经到时的节点仍会补生成；连续生成失败
                  3 次会自动暂停。
                </p>
              </HelpFigureBlock>
            </div>
          ...
          <section data-section-id="calendar" class="help-section">
            ...
            >
              <HelpFigureBlock title="新建与管理便签">
                <template #figure><MockNewNotePanel /></template>
                <p>
                  点击日期格左下角＋，或日期侧栏右上角＋，打开与便签列表一致的新建表单。选择今天且不调整时间时立即生效；选择过去或未来日期时默认使用当天
                  00:01，仍可手动修改。
                </p>
                <p>
                  过去日期会按历史补录创建，直接进入「进行中」且不发送系统提醒；创建时间和修改时间仍记录实际补录时刻。创建或修改成功后，日期横条、当天数量和侧栏卡片会自动同步，不需要手动刷新。
                </p>
              </HelpFigureBlock>
            </div>
